...
puts "Il y a #{journalists.size} journalistes dans ce tableau."

#Question 2
puts "Combien d'entrées contiennent un numéro?"
count = 0 #définition d'uine variable d'incrémentation
...
#Question 3
puts "Combien d'entrées contiennent les 4 lettres du prénom Aude à la suite (sans prendre en compte les majuscules)?"
count = 0
journalists.each do |email|
...
#Question 6
puts "Combien de _ dans tous les pseudo confondus?"
count = 0
journalists.each do |email|
    if email.match (/_/)
        count +=1
...
#Question 7
puts "Tri de la liste d'entrées par ordre alphabétique."
journalists_sans = []
journalists.each do |email|
    journalists_sans.push(email.gsub(/[@_]/, "").capitalize)
end
